...
def render():
    ...
    wave_object = wave.open("files/" + input_file.get())
    ...
    def drawF(frameno: int):
        ...
        for bar in range(0, int(bars.get())):
            ...
            height_gap = 1 - height
            fill_brightness = int(255 * math.pow(signal_fraction, float(brightness_exp.get())))
            
            draw.rectangle(
                (
                    (width_gap * float(bar_justify_x.get()) + width * bar) * bg_width + math.floor(int(bar_spacing.get()) / 2),
                    (height_gap * float(bar_justify_y.get())) * bg_height,
                    (width_gap * float(bar_justify_x.get()) + width * (bar + 1)) * bg_width - math.ceil(int(bar_spacing.get()) / 2),
